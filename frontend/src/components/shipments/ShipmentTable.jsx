					...
					{shipments.map((shipment) => (
						<tr key={shipment.shipment_id}>
							...
							<td>
								...
								<div style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{shipment.origin}</div>
							</td>
							<td>
								...
								<div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
									...
									{hideScore ? (
										...
										<Badge level={shipment.current_risk_level} size="sm">
											{shipment.current_risk_score}
										</Badge>
									)}
								</div>
							</td>
							<td className="mono">{formatDate(shipment.expected_arrival)}</td>
							<td>
								<button type="button" className="btn-outline" onClick={() => onTrack?.(shipment.shipment_id)}>
									Track
									<ArrowRight size={14} />
								</button>
							</td>
